...
const INDEX_NAME = import.meta.env.VITE_ALGOLIA_INDEX_NAME || 'pc_components'
...
function createTypeFilter(componentType) {
  if (!componentType) return undefined
  const safe = String(componentType).replace(/\\/g, '\\\\').replace(/"/g, '\\"')
  return `type:"${safe}"`
}
      ...
      <div className="flex items-start justify-between gap-4">
        ...
        <div className="text-right flex-shrink-0">
          {Number.isFinite(price) && (
            <div className="text-white font-bold text-sm">${price.toFixed(2)}</div>
          )}
          {hit.socket && (
...
export default function AlgoliaSearch({
  ...
  const typeFilter = useMemo(() => createTypeFilter(componentType), [componentType])
    ...
    <div className="relative">
      ...
      >
        {isAlgoliaConfigured ? (
          ...
          <>Search (Algolia not configured) {componentType ? `(${componentType})` : ''}...</>
        )}
      </button>
          ...
          <div className="fixed inset-x-4 top-20 md:left-1/2 md:-translate-x-1/2 md:max-w-2xl z-50 bg-slate-900/95 backdrop-blur-xl rounded-2xl border border-white/10 shadow-2xl max-h-[70vh] flex flex-col">
            {isAlgoliaConfigured ? (
              <InstantSearch searchClient={searchClient} indexName={indexName}>
                <Configure hitsPerPage={10} {...(typeFilter && { filters: typeFilter })} />
              ...
              <>
                <div className="p-4 border-b border-white/10 flex items-center justify-between gap-3">
                  <div className="text-white font-semibold">Algolia search not configured</div>
                  <button
                    onClick={() => setIsOpen(false)}
                    className="p-2 text-slate-400 hover:text-white hover:bg-white/5 rounded-lg transition-colors"
                  >
                ...
                <div className="p-4 text-sm text-slate-300 space-y-2">
                  <p>Set these variables in <code className="text-slate-200">frontend/.env</code> then restart the dev server:</p>
                  <ul className="list-disc pl-5 text-slate-300 space-y-1">
                    <li><code className="text-slate-200">VITE_ALGOLIA_APP_ID</code></li>
                    <li><code className="text-slate-200">VITE_ALGOLIA_SEARCH_KEY</code></li>
                    <li><code className="text-slate-200">VITE_ALGOLIA_INDEX_NAME</code> (optional, default: <code className="text-slate-200">pc_components</code>)</li>
                  </ul>
                  <p className="text-slate-400">The rest of the builder still works via the backend endpoints.</p>
                </div>
              </>
            )}
          </div>
